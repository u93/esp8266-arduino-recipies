...
#include <WiFiClientSecure.h> //enable HTTPS to handle Secure Communication to Cloud Services like API Gateway
...
//Secure Communication + API Gateway variables, constants, and instances
const char check_host[] = "**********.execute-api.us-east-1.amazonaws.com"; //Set your API Gateway Endpoint
WiFiClientSecure secureclient; //Creates an instance of WiFiClientSecure with the purpose of using HTTPS to connect to API Gateway
int CHIP_ID; //Variable to define the ESP8266 Chip ID
...
void setup() {
  ...
  //Get unix time to sends in communication with API Gateway 
  sendNTPpacket (int_chk);
  unix_time = readNTPpacket();
  CHIP_ID = ESP.getChipId(); //USe ESP specific APIs to get the Chip ID
  delay (10);

  APIGW_check(unix_time, CHIP_ID); //Connects to AWS API Gateway usign HTTPS and passing parameters that later can be used to make a Lambda work 
  Networking.attach(10, udp_tcp_Networking_flag); //Activate UDP + TCP functionality every 10 seconds
}
...
char APIGW_check(long int current_time, int chip_id){
  delay (10);
  if (secureclient.connect(check_host, 443)) //Connects to the host defined above using port 443
  {
    Serial.println("API Gateway Connected");
    String URL = String("/dev/develop/esp8266?time=") + current_time + String("&id=") + chip_id; //As part of the complete URI, you have to enter the rest of the endpoint for API Gateway that you defined when deploying in AWS
    Serial.println(URL);
   ...
   while (!secureclient.available()) { //Waits for the connection to be available to start printing the response
      delay(100);
   }
   while (secureclient.connected()) //Reads and print response of the server while itś available and connected
    {
      Ticker HELP; //Allows activation of the help function to activate a disconnection if there is huge delay while communicating to API Gateway
      HELP.attach (5, help);
